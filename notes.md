...
Things to do for Mark session:
  ...
  * Fix stuck note-off in Q problem
  * Build rudimentary UI for live_gen
  
  * GUI: Note density (% chance any given note is played)

  * GUI: Time filter - only allow notes up to a certain time in the future 

  * GUI: time frame: do not use ring buffer, instead just capture everything in a time frame without wrapping. could just make ring buffer very long
