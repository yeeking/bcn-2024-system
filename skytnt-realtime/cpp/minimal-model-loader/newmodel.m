...
+ (nullable NSURL *)URLOfModelInThisBundle {
    NSString *assetPath = [[NSBundle bundleForClass:[self class]] pathForResource:@"newmodel" ofType:@"mlmodelc"];
    if (nil == assetPath) { os_log_error(OS_LOG_DEFAULT, "Could not load newmodel.mlmodelc in the bundle resource"); return nil; }
...
+ (void)loadContentsOfURL:(NSURL *)modelURL configuration:(MLModelConfiguration *)configuration completionHandler:(void (^)(newmodel * _Nullable model, NSError * _Nullable error))handler {
    [MLModel loadContentsOfURL:modelURL
        ...
        if (model != nil) {
            newmodel *typedModel = [[newmodel alloc] initWithMLModel:model];
...
- (nullable newmodelOutput *)predictionFromFeatures:(newmodelInput *)input options:(MLPredictionOptions *)options error:(NSError * _Nullable __autoreleasing * _Nullable)error {
    ...
    return [[newmodelOutput alloc] initWithVar_1661:(MLMultiArray *)[outFeatures featureValueForName:@"var_1661"].multiArrayValue];
}

- (void)predictionFromFeatures:(newmodelInput *)input completionHandler:(void (^)(newmodelOutput * _Nullable output, NSError * _Nullable error))completionHandler {
    [self.model predictionFromFeatures:input completionHandler:^(id<MLFeatureProvider> prediction, NSError *predictionError) {
        if (!prediction) {
            ...
            newmodelOutput *output = [[newmodelOutput alloc] initWithVar_1661:(MLMultiArray *)[prediction featureValueForName:@"var_1661"].multiArrayValue];
            completionHandler(output, predictionError);
        }
    }];
}

- (void)predictionFromFeatures:(newmodelInput *)input options:(MLPredictionOptions *)options completionHandler:(void (^)(newmodelOutput * _Nullable output, NSError * _Nullable error))completionHandler {
...
- (nullable newmodelOutput *)predictionFromX_1:(MLMultiArray *)x_1 error:(NSError * _Nullable __autoreleasing * _Nullable)error {
    newmodelInput *input_ = [[newmodelInput alloc] initWithX_1:x_1];
...
- (nullable NSArray<newmodelOutput *> *)predictionsFromInputs:(NSArray<newmodelInput*> *)inputArray options:(MLPredictionOptions *)options error:(NSError * _Nullable __autoreleasing * _Nullable)error {
    ...
    NSMutableArray<newmodelOutput*> *results = [NSMutableArray arrayWithCapacity:(NSUInteger)outBatch.count];
    for (NSInteger i = 0; i < outBatch.count; i++) {
        id<MLFeatureProvider> resultProvider = [outBatch featuresAtIndex:i];
        newmodelOutput * result = [[newmodelOutput alloc] initWithVar_1661:(MLMultiArray *)[resultProvider featureValueForName:@"var_1661"].multiArrayValue];
