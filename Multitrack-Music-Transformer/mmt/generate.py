...
def save_result(filename, data, sample_dir, encoding):
    ...
    music.write_audio(
        sample_dir / "wav" / f"{filename}.wav",
        # soundfont_path="/home/matthew/src/ai-music/soundfonts/SGM-v2.01-NicePianosGuitarsBass-V1.2.sf2"
        soundfont_path="/home/matthew/src/ai-music/soundfonts/yamaha tyros 4_just_t4_fixed.sf2"
        # options="-o synth.polyphony=4096",
    )
    ...
    subprocess.check_output(
        ...
        + ["-b:a", "192k"]
        + [str(sample_dir / "mp4" / f"{filename}.mp4")]
    )
    ...
    subprocess.check_output(
        ...
        + ["-b:a", "192k"]
        + [str(sample_dir / "mp4-trimmed" / f"{filename}.mp4")]
    )
...
def main():
    ...
    with torch.no_grad():
        ...
        for i in tqdm.tqdm(range(args.n_samples), ncols=80):
            ...
            # Ground truth
            # ------------
            logging.info(f"--- Saving ground truth ---")
            ...
            tgt_start[:, 0, 0] = sos
            logging.info(f"--- unconditional generation ---")
            ...
            logging.info(f"--- instrument informed generation ---")
            ...
            logging.info(f"--- 4-beat continuation generation ---")
            ...
            logging.info(f"--- 16-beat continuation generation ---")
